...
//from getopt FIX, from canvas https://canvas.kth.se/courses/20917/discussion_topics/146002
...
void shared_sgemm_kernel(float *C, float *A, float *B, long size)
{
	...
	//Shared mem for tile_B
	__shared__ float tile_B[TILE_SIZE][TILE_SIZE];
	//Shared mem for tile_A
	...
	if (col < size && row < size) {
		...
		for (long m = 0; m < size / TILE_SIZE; ++m) {
			...
			//description of unroll can be find here https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html
			#pragma unroll
			for (long k = 0; k < TILE_SIZE; ++k) {
				/* TODO Perform multiplication here */
				// multiply rows with columns
				val +=  tile_B[k][threadIdx.x] * tile_A[threadIdx.y][k];
			}
			__syncthreads();
		}
...
void cublas_sgemm(float *C, float *A, float *B, long size)
{
	...
	/*
	   Parameters to cublasSgemm
	   m n k (should all be size)
	   alpha
	   A lda
	   B ldb
	   beta

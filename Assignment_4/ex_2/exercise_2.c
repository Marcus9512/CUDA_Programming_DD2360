...
void saxpy_on_cpu(float* x, float* y, float a, int length) {
    clock_t start = clock();
...
int main(int argc, char* argv) {
    ...
    //Init
    int array_size_byte = ARRAY_SIZE * sizeof(float);
    float* x = (float*)malloc(array_size_byte);
    float* y = (float*)malloc(array_size_byte);
    float* res_gpu = (float*)malloc(array_size_byte);
    float a = 2;
    float len = ARRAY_SIZE;
    ...
    cl_mem x_dev = clCreateBuffer(context, CL_MEM_READ_ONLY, array_size_byte, NULL, &err);CHK_ERROR(err);
    cl_mem y_dev = clCreateBuffer(context, CL_MEM_READ_ONLY, array_size_byte, NULL, &err);CHK_ERROR(err);
 

    clock_t start = clock();

    //Transfer data to host
    err = clEnqueueWriteBuffer(cmd_queue, x_dev, CL_TRUE, 0, array_size_byte,x,0, NULL, NULL);CHK_ERROR(err);
    err = clEnqueueWriteBuffer(cmd_queue, y_dev, CL_TRUE, 0, array_size_byte,y,0, NULL, NULL);CHK_ERROR(err);
   
    cl_program program = clCreateProgramWithSource(context, 1, (const char **)&saxpyKernel, NULL, &err);CHK_ERROR(err);
    ...
    err = clSetKernelArg(kernel, 3, sizeof(float), (void*)&len);CHK_ERROR(err);
  
    size_t n_workitem = ARRAY_SIZE + (THREADS - (ARRAY_SIZE % THREADS));
    size_t workgroup_size = THREADS ;
    ...
    err = clEnqueueReadBuffer(cmd_queue, y_dev, CL_TRUE, 0, array_size_byte, res_gpu, 0, NULL, NULL);CHK_ERROR(err);
    ...
    printf("Computing SAXPY on the GPU Done in %f seconds, only kernel time %f\n", time, time2);
    ...
    bool results = isSame(y, res_gpu, ARRAY_SIZE);
